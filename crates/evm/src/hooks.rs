...
impl<C: sov_modules_api::Context> Evm<C> {
    ...
    pub fn begin_soft_confirmation_hook(
        ...
        // Soft confirmation won't have da info after fork2
        if soft_confirmation_info.current_spec < CitreaSpecId::Fork2 {
            self.last_l1_hash
    ...
    pub fn end_soft_confirmation_hook(
        ...
        let header = AlloyHeader {
            ...
            blob_gas_used: if citrea_spec_id_to_evm_spec_id(current_spec) >= SpecId::CANCUN {
                Some(0)
            } else {
                None
            },
            excess_blob_gas: if citrea_spec_id_to_evm_spec_id(current_spec) >= SpecId::CANCUN {
                Some(0)
            } else {
                None
            },
