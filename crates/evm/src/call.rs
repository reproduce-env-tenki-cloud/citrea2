...
use sov_modules_api::{
    native_error, CallResponse, DaSpec, SoftConfirmationModuleCallError, SpecId as CitreaSpecId,
    WorkingSet,
};
...
impl<C: sov_modules_api::Context, Da: DaSpec> Evm<C, Da> {
    ...
    pub(crate) fn execute_system_events(
        ...
        let db: EvmDb<'_, C, Da> = self.get_db(working_set, citrea_spec);
    ...
    pub(crate) fn execute_call(
        ...
        let evm_db: EvmDb<'_, C, Da> = self.get_db(working_set, context.active_spec());
