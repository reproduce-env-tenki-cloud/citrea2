...
async fn rpc_server() -> (jsonrpsee::server::ServerHandle, SocketAddr) {
    ...
    let rpc_module = create_rpc_module(db);
...
async fn getters_succeed() {
    ...
    rpc_client
        .get_proofs_by_slot_height(U64::from(0))
        .await
        .unwrap();

    rpc_client.get_proofs_by_slot_hash(hash).await.unwrap();
    ...
    rpc_client
        .get_verified_proofs_by_slot_height(U64::from(0))
        .await
        .unwrap();
